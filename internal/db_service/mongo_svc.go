...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("PN_REGISTRY_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("PN_REGISTRY_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("PN_REGISTRY_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("PN_REGISTRY_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("PN_REGISTRY_API_MONGODB_DATABASE", "pn-registry")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("PN_REGISTRY_API_MONGODB_COLLECTION", "record")
	}

	if svc.Timeout == 0 {
		seconds := enviro("PN_REGISTRY_API_MONGODB_TIMEOUT_SECONDS", "10")
